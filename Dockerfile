...
RUN mkdir /tmp/helm && cd /tmp/helm \
  && wget --no-check-certificate https://get.helm.sh/helm-v3.6.0-linux-amd64.tar.gz \
...
RUN mkdir /tmp/kubectl && cd /tmp/kubectl \
  && wget --no-check-certificate https://storage.googleapis.com/kubernetes-release/release/v1.21.1/bin/linux/amd64/kubectl \
  && chmod +x ./kubectl \
